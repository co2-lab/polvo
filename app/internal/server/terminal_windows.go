...
// session holds a running ConPTY process and its scrollback buffer.
type session struct {
	mu         sync.Mutex
	cpty       *conpty.ConPty
	scrollback []byte
...
func (s *Server) handleTerminalWS(w http.ResponseWriter, r *http.Request) {
	...
	// Resume existing session if ConPTY is still alive.
	...
	// Resolve the shell/command to run.
	...
	cpty, err := conpty.New(80, 24, 0)
	if err != nil {
		return
	}

	env := append(os.Environ(), "TERM=xterm-256color")
	attr := &syscall.ProcAttr{Env: env}
	_, handle, err := cpty.Spawn(exe, []string{exe}, attr)
	if err != nil {
		cpty.Close()
		return
	}

	sess := &session{cpty: cpty}
	s.term.set(id, sess)
	_ = send(msgNew, nil)

	go func() {
		proc, err := os.FindProcess(int(handle))
		if err == nil {
			_, _ = proc.Wait()
		}
		cpty.Close()
	...
	for {
		...
		case msgResize:
			...
			if json.Unmarshal(msg[1:], &size) == nil {
				_ = sess.cpty.Resize(int(size.Cols), int(size.Rows))
			}
		}
	}
}
