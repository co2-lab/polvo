...
import type { LayoutNode, PanelNode, SplitNode, Workspace, IDEProject, DockItem, SplitDirection, DockPosition, ThemeDef, PanelKind, KindBehavior } from '../types/ide'
...
interface IDEState {
  ...
  addPanel: (contentId: string, targetPanelId?: string, position?: 'top'|'bottom'|'left'|'right', tabProjectId?: string) => void
...
export function getKind(contentId: string): PanelKind {
  if (contentId.startsWith('file:') || contentId.startsWith('newfile:') || contentId === 'editor') return 'editor'
...
const initialWorkspaces: Workspace[] = [
  { id: 'ws-1', name: 'Workspace 1', layout: initialLayout }
]
...
const predefinedThemes: ThemeDef[] = [
  {
    ...
    name: 'Midnight Aura',
    type: 'predefined',
    colors: { bg: '#0B0F19', surface: '#111827', border: '#1F2937', accent: '#38BDF8', text: '#F3F4F6' }
  },
  {
    id: 'theme-tokyo',
    name: 'Tokyo Night',
    type: 'predefined',
    colors: { bg: '#1A1B26', surface: '#24283B', border: '#414868', accent: '#BB9AF7', text: '#C0CAF5' }
  },
  {
    id: 'theme-monokai',
    name: 'Monokai Vibrant',
    type: 'predefined',
    colors: { bg: '#2D2A2E', surface: '#3A3839', border: '#5B595C', accent: '#FF6188', text: '#FCFCFA' }
  },
  {
    id: 'theme-cyberpunk',
    name: 'Cyberpunk',
    type: 'predefined',
    colors: { bg: '#09090B', surface: '#18181B', border: '#27272A', accent: '#FDE047', text: '#FAFAFA' }
  },
  {
    id: 'theme-dark',
    name: 'Vercel Dark',
    type: 'predefined',
    colors: { bg: '#000000', surface: '#0A0A0A', border: '#222222', accent: '#EDEDED', text: '#888888' }
  },
  {
    id: 'theme-light',
    name: 'Clean Light',
    type: 'predefined',
    colors: { bg: '#F8FAFC', surface: '#FFFFFF', border: '#E2E8F0', accent: '#6366F1', text: '#0F172A' }
  }
]
...
function findPlacement(root: LayoutNode): PlacementResult {
  ...
  const allSameParent = candidates.length > 1 &&
...
const findPanel = (node: LayoutNode, pred: (contentId: string, n: PanelNode) => boolean): PanelNode | null => {
...
const replaceNode = (node: LayoutNode, targetId: string, replacer: (node: LayoutNode) => LayoutNode): LayoutNode => {
...
const removeNodeFromTree = (node: LayoutNode, targetId: string): LayoutNode | null => {
  ...
  if (node.type === 'split') {
    ...
    if (newChildren.length === 1) {
      ...
      if (survivor.type === 'panel') return { ...survivor, id: node.id, sessionId: survivor.sessionId ?? survivor.id }
...
export const useIDEStore = create<IDEState>()(persist((set) => ({
  ...
  shortcuts: [
    ...
    { id: 'cmd-shift-f', name: 'Global Search', context: 'Global', keys: ['Ctrl', 'Shift', 'F'] },
  ...
  toggleDockItemVisibility: (id) => set((state) => ({
    dockItems: state.dockItems.map(item => item.id === id ? { ...item, hidden: !item.hidden } : item)
  })),
  ...
  addWorkspace: () => set((state) => {
    const newWs: Workspace = { id: crypto.randomUUID(), name: `Workspace ${state.workspaces.length + 1}`, layout: null }
  ...
  removeWorkspace: (id) => set((state) => {
    ...
    const activeId = state.activeWorkspaceId === id ? remaining[0].id : state.activeWorkspaceId
    return { workspaces: remaining, activeWorkspaceId: activeId }
  }),
  togglePinWorkspace: (id) => set((state) => ({
  ...
  duplicateWorkspace: (id) => set((state) => {
    ...
    if (!src) return state
    const newWs: Workspace = { id: crypto.randomUUID(), name: `${src.name} (copy)`, layout: src.layout }
  ...
  addPanel: (contentIdRaw, targetPanelId, position, tabProjectId) => set((state) => {
    ...
    const newPanel: PanelNode = { id: newPanelId, sessionId: newPanelId, type: 'panel', contentId, tabs: [contentId], tabProjects: tabProjectId ? { [contentId]: tabProjectId } : undefined, kind, projectId: resolvedProjectId }
    ...
    if (!ws.layout) {
      ...
      newLayout = replaceNode(ws.layout, targetPanelId, (targetNode) => {
        ...
        const innerPanel: PanelNode = { ...t, id: crypto.randomUUID(), sessionId: t.sessionId ?? t.id }
        const splitNode: SplitNode = { id: t.id, type: 'split', direction, children: isFirst ? [newPanel, innerPanel] : [innerPanel, newPanel] }
        return splitNode
      })
    } else {
      ...
      if (placement.kind === 'split-group') {
        newLayout = replaceNode(ws.layout, placement.groupId, (groupNode) => ({
          ...
          id: groupNode.id, type: 'split', direction: placement.direction, children: [groupNode.type === 'split' ? { ...groupNode, id: crypto.randomUUID() } : groupNode, newPanel],
        }))
      } else {
        newLayout = replaceNode(ws.layout, placement.panelId, (targetNode) => {
          const t = targetNode as PanelNode
          const innerPanel: PanelNode = { ...t, id: crypto.randomUUID(), sessionId: t.sessionId ?? t.id }
          return { id: t.id, type: 'split', direction: placement.direction, children: [innerPanel, newPanel] }
        })
      }
    }
  ...
  markDirty: (contentId) => set((state) => {
    const next = new Set(state.dirtyFiles)
    next.add(contentId)
    return { dirtyFiles: next }
  }),
  markClean: (contentId) => set((state) => {
    const next = new Set(state.dirtyFiles)
    next.delete(contentId)
    return { dirtyFiles: next }
  }),
  replaceTabContent: (panelId, oldContentId, newContentId) => set((state) => {
    const wsIndex = state.workspaces.findIndex(w => w.id === state.activeWorkspaceId)
    if (wsIndex === -1) return state
    const ws = state.workspaces[wsIndex]
    ...
    const newLayout = replaceNode(ws.layout, panelId, (n) => {
      ...
      return { ...n, contentId: n.contentId === oldContentId ? newContentId : n.contentId, tabs: newTabs }
    })
    const newWorkspaces = [...state.workspaces]
    newWorkspaces[wsIndex] = { ...ws, layout: newLayout }
  ...
  switchTab: (panelId, contentId) => set((state) => {
    const wsIndex = state.workspaces.findIndex(w => w.id === state.activeWorkspaceId)
    if (wsIndex === -1) return state
    const ws = state.workspaces[wsIndex]
    if (!ws.layout) return state
  ...
  closeTab: (panelId, contentId) => set((state) => {
    const wsIndex = state.workspaces.findIndex(w => w.id === state.activeWorkspaceId)
    if (wsIndex === -1) return state
    const ws = state.workspaces[wsIndex]
    if (!ws.layout) return state
    ...
    const newContentId = panel.contentId === contentId
  ...
  setKindBehavior: (kind, behavior) => set((state) => ({
  ...
  pinExplorer: (panelId, projectId) => set((state) => ({
    workspaces: state.workspaces.map(ws => ({
      ...ws,
      layout: ws.layout ? setPanelProp(ws.layout, panelId, { pinnedProjectId: projectId }) : null,
    })),
  })),
  unpinExplorer: (panelId) => set((state) => ({
    workspaces: state.workspaces.map(ws => ({
      ...ws,
      layout: ws.layout ? setPanelProp(ws.layout, panelId, { pinnedProjectId: undefined }) : null,
    })),
  })),
  setPanelSizes: (groupId, layout) => set((state) => ({
  ...
  migrate: (persisted: unknown, version: number) => {
    ...
    if (version < 8) {
      ...
      const dockItems = Array.isArray(state.dockItems)
        ? (state.dockItems as Array<Record<string, unknown>>).filter(i => !removedIds.has(i.id as string))
        : state.dockItems
    ...
    if (version < 3) {
      const migrateLayout = (node: unknown): unknown => {
        ...
        if (n.type === 'panel') {
          return { ...n, tabs: Array.isArray(n.tabs) ? n.tabs : [n.contentId], kind: getKind(n.contentId as string) }
        }
    ...
    if (version < 5) {
      ...
      const sessions = Array.isArray(state.diffSessions)
        ? (state.diffSessions as Array<Record<string, unknown>>).map(s => ({
            id: s.id, name: s.name, left: undefined, right: undefined
          }))
        : []
